...
SplayNode *insertnode(SplayNode *p, int val) {
  if (p == nullptr) {
    p = new SplayNode(val);  // 生成新节点
    return p;
  } else if (p->data == val) {  // 不插入
    return p;
  } else if (p->data > val) {  // 左子树中插入
    p->lchild = insertnode(p->lchild, val);
  } else {  // 右子树中插入
    p->rchild = insertnode(p->rchild, val);
  }
...
SplayNode *deletenode(SplayNode *&p, int val) {
  ...
  if (newroot != nullptr) {  // 找到值为val的节点
    // 实现分裂
    SplayNode *left = p->lchild;
    SplayNode *right = p->rchild;
    printf("pval=%d val=%d\n", p->data, val);
    assert(left == nullptr);
