...
bool StatCheck(const char* buffer, const int len, int& num) {
  int C = 0, O = 0, W = 0;
...
inline bool CheckHash(const char* buffer, const int len, int& tmpHash,
                      vector<int>& cs, vector<int>& os, vector<int>& ws) {
  ll pre = 0;
  tmpHash = 0;
  for (int i = 0; i < len; i++) {
    const char c = buffer[i];
...
bool Match(const char* buffer, const int L, const int R, const int len,
  ...
  vector<int> os;
  vector<int> ws;
  if (!CheckHash(buffer, len, tmpHash, cs, os, ws)) return false;
  if (k == 0) return true;

  if (matchHash[L][R][k].count(tmpHash)) return false;
  matchHash[L][R][k].insert(tmpHash);

  for (auto oit = os.begin(); oit != os.end(); oit++) {
    const int oi = *oit;
    // if (buffer[oi] != 'O') continue;
    for (auto cit = cs.begin(); cit != cs.end() && *cit < oi; cit++) {
      const int ci = *cit;
      // if (buffer[ci] != 'C') continue;
      for (auto wit = ws.rbegin(); wit != ws.rend() && *wit > oi; wit++) {
        const int wi = *wit;
...
/*
Executing...
   Test 1: TEST OK [0.004 secs limit:2s, 3560 KB]
   Test 2: TEST OK [0.011 secs limit:2s, 3748 KB]
   Test 3: TEST OK [0.007 secs limit:2s, 3544 KB]
   Test 4: TEST OK [0.007 secs limit:2s, 3600 KB]
   Test 5: TEST OK [0.007 secs limit:2s, 3564 KB]
   Test 6: TEST OK [0.095 secs limit:2s, 3748 KB]
   Test 7: TEST OK [0.007 secs limit:2s, 3576 KB]
   Test 8: TEST OK [0.074 secs limit:2.5s, 3544 KB]
   Test 9: TEST OK [1.169 secs limit:2s, 4136 KB]
   Test 10: TEST OK [1.400 secs limit:2s, 4176 KB]
