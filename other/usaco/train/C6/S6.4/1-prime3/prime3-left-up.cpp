...
void Dfs(int row, int col, int k, int preVal) {
  ...
  for (int v = startIndex; v < 10; v++) {
    ...
    if (p == -1) {
      // if (preVal == 25122512) printf("v=%d no next\n", v);
      continue;
    }
    // 选择 v
    ...
    if (row + col == 4 && !Check(UP, v, preVal)) {
      // if (preVal == 25122512) printf("UP check no pass, UP=%d v=%d\n", UP,
      // v);
      continue;
    }
    ...
    Dfs(row - 1, col + 1, k + 1, preVal * 10 + v);
...
/*
Executing...
   Test 1: TEST OK [0.025 secs limit:2s, 3328 KB]
   Test 2: TEST OK [0.063 secs limit:2s, 3316 KB]
   Test 3: TEST OK [0.193 secs limit:2s, 3376 KB]
   Test 4: TEST OK [0.840 secs limit:2s, 3392 KB]
   Test 5: TEST OK [0.686 secs limit:2s, 3372 KB]
   Test 6: TEST OK [1.673 secs limit:2s, 3380 KB]
   Test 7: TEST OK [1.792 secs limit:2s, 3320 KB]
  > Run 8: Execution error: Your program (`prime3') used more than the
        allotted runtime of 2 seconds (it ended or was stopped at 2.107
        seconds) when presented with test case 8. It used 3400 KB of
        memory. 

        ------ Data for Run 8 [length=5 bytes] ------
        23 7 
        ----------------------------
    Test 8: RUNTIME 2.107>2 (3400 KB)
*/