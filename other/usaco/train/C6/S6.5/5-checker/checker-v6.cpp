...
void Dfs1(const int p) {  // 枚举第 p 行
  ...
  for (int i = 1; i <= N; i++) {
    if (cols[i]) continue;
    if (g[row][i]) continue;
    const auto [x0, y0] = Trans0(row, i);
    const auto [x1, y1] = Trans1(row, i);
    if (LU_RD[x0][y0]) continue;
    if (RU_LD[x1][y1]) continue;
...
/*
Executing...
   Test 1: TEST OK [0.000 secs limit:1s, 1740 KB]
   Test 2: TEST OK [0.004 secs limit:1s, 1728 KB]
   Test 3: TEST OK [0.004 secs limit:1s, 1704 KB]
   Test 4: TEST OK [0.011 secs limit:1s, 1780 KB]
   Test 5: TEST OK [0.025 secs limit:1s, 1720 KB]
   Test 6: TEST OK [0.077 secs limit:1s, 1784 KB]
   Test 7: TEST OK [0.469 secs limit:1s, 1732 KB]
  > Run 8: Execution error: Your program (`checker') used more than
        the allotted runtime of 1 seconds (it ended or was stopped at
        1.820 seconds) when presented with test case 8. It used 1744 KB of
        memory. 

        ------ Data for Run 8 [length=3 bytes] ------
        13 
        ----------------------------
    Test 8: RUNTIME 1.820>1 (1744 KB)
