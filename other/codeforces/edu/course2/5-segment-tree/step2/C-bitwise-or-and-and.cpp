/*
ID: tiankonguse
TASK: C. Bitwise OR and AND C. 按位或和与
LANG: C++
MAC EOF: ctrl+D
link:
...
*/
#define TASK "C-bitwise-oR-and-AND"
#define TASKEX ""
...
struct SegTree {
  ...
  void PushDown(int rt) {
    if (sign[rt]) {
      sign[rt << 1] = (sign[rt << 1] | sign[rt]);
      sign[rt << 1 | 1] = (sign[rt << 1 | 1] | sign[rt]);

      sumVal[rt << 1] = sign[rt] | sumVal[rt << 1];
      sumVal[rt << 1 | 1] = sign[rt] | sumVal[rt << 1 | 1];
