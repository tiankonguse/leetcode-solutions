/*
ID: tiankonguse
TASK: F. Assignment and Sum  F. 赋值和求和
LANG: C++
MAC EOF: ctrl+D
link:
...
*/
#define TASK "F-assignment-and-sum"
#define TASKEX ""
...
struct SegTree {
  ...
  void PushUp(int rt, int l, int r) {
    opVal[rt] = Op(opVal[rt << 1], opVal[rt << 1 | 1]);
  }
  void PushDown(int rt, int l, int r) {
    if (sign[rt]) {
      ...
      int m = (l + r) >> 1;
      opVal[rt << 1] = sign[rt] * (m - l + 1);
      opVal[rt << 1 | 1] = sign[rt] * (r - m);
  ...
  ll QueryMin(int L, int R, int l = 1, int r = maxNM, int rt = 1) {
    ...
    ll ret = 0;
    if (L <= m) {
      ret = Op(ret, QueryMin(L, R, lson));
    }
    if (m < R) {
      ret = Op(ret, QueryMin(L, R, rson));
    }
    return ret;
  }
};
