/*
ID: tiankonguse
TASK: B. Multiplication and Sum B. 乘法和求和
LANG: C++
MAC EOF: ctrl+D
link:
...
*/
#define TASK "B-multiplication-and-sum"
#define TASKEX ""
...
struct SegTree {
  ...
  void Init(int n) {
    ...
    sumVal.resize(maxNM << 2, 1);
  }

  // 合并函数，按需进行合并
  void PushUp(int rt, int l, int r) {
    sumVal[rt] = (sumVal[rt << 1] + sumVal[rt << 1 | 1]) % mod_10_9_7;
  }
  void PushDown(int rt) {
    if (sign[rt] > 1) {
      sign[rt << 1] = (sign[rt << 1] * sign[rt]) % mod_10_9_7;
      sign[rt << 1 | 1] = (sign[rt << 1 | 1] * sign[rt]) % mod_10_9_7;

      sumVal[rt << 1] = (sign[rt] * sumVal[rt << 1]) % mod_10_9_7;
      sumVal[rt << 1 | 1] = (sign[rt] * sumVal[rt << 1 | 1]) % mod_10_9_7;
  ...
  void Update(int L, int R, ll add, int l = 1, int r = maxNM, int rt = 1) {
    if (add == 1) return;
    if (L <= l && r <= R) {
      sign[rt] = (sign[rt] * add) % mod_10_9_7;
      sumVal[rt] = (sumVal[rt] * add) % mod_10_9_7;
      return;
    }
    PushDown(rt);
    int m = (l + r) >> 1;
  ...
  ll QuerySum(int L, int R, int l = 1, int r = maxNM, int rt = 1) {
    ...
    ll ret = 0;
    if (L <= m) {
      ret = (ret + QuerySum(L, R, lson)) % mod_10_9_7;
    }
    if (m < R) {
      ret = (ret + QuerySum(L, R, rson)) % mod_10_9_7;
    }
    return ret;
  }
};
