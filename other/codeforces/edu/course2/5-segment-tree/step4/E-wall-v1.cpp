...
struct SegTree {
  ...
  void Update(int L, int R, int op, ll v, int l = 1, int r = maxNM, int rt = 1) {
    if (L <= l && r <= R) {
      pair<int, ll> ops = {op, v};
      UpdateNode(rt, l, r, {ops});
      return;
    }
    PushDown(rt, l, r);
    int m = (l + r) >> 1;
  ...
  ll QuerySum(int L, int R, int l = 1, int r = maxNM, int rt = 1) {
    if (L <= l && r <= R) {
      return signVal[rt].ops.front().second;
    }
    PushDown(rt, l, r);
    int m = (l + r) >> 1;
    if (R <= m) {
...
void Solver() {  //
  ...
  while (m--) {
    ...
    segTree.Update(l + 1, r + 1, op, d);
  }
  for (int i = 1; i <= n; i++) {
    printf("%lld\n", segTree.QuerySum(i, i));
  }
}
