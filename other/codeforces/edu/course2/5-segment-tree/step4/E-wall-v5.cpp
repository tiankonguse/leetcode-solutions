...
DESC:
  ...
  与 V4 的区别是， Node 中数组改为 pair。
*/
#define TASK "E-wall-v5"
#define TASKEX ""
...
void Merge(const pair<int, ll>& os, pair<int, ll>& ops) {
  auto& o3 = os.first;   // UP
  auto& o4 = os.second;  // DOWN

  auto& o1 = ops.first;   // UP
  auto& o2 = ops.second;  // DOWN
  ...
  if (o3 == o4) {  // 新值固定
    ...
    // TODO: 旧值固定
    // 贪心的反例 case: 此时先升高再降低 与 先降低再升高 答案是不同的
    // 只有一次时，升高为 {INT_MAX, d}， 降低为 {d, 0}, 操作后值已经固定，没有问题
    // 分析：什么时候会出现两次 {d1, d2}
    if (o3 > o1) {
      o1 = o2 = o3;
    }
...
struct SegTree {
  ...
  void UpdateNode(int rt, int l, int r, const pair<int, ll>& o) {
    flag[rt] = 1;
