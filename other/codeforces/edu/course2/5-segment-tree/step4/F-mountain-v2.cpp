...
DESC: 如果高低降低到0，修正校准到0
*/
#define TASK "F-mountain-v2"
#define TASKEX ""
...
struct SegTree {
  ...
  ll QueryMax(int L, int R, int l = 1, int r = maxNM, int rt = 1) {
    ...
    if (R <= m) {
      ...
      return max(disVal[rt << 1], 0ll) + QueryMax(L, R, rson);
    } else {
      return max(QueryMax(L, R, lson), max(disVal[rt << 1], 0ll) + QueryMax(L, R, rson));
    }
  }
};
