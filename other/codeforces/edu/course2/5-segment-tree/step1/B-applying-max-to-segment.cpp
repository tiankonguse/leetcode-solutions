/*
ID: tiankonguse
TASK: B. Applying MAX to Segment B. 将 MAX 应用于分段
LANG: C++
MAC EOF: ctrl+D
link:
https://codeforces.com/edu/course/2/lesson/5/1/practice/contest/279634/problem/B
PATH: ITMO 学院：试点课程 » 线段树，第 2 部分 » 步骤1 » 实践
submission: https://codeforces.com/edu/course/2/lesson/5/1/practice/contest/279634/submission/298745978
*/
#define TASK "B-applying-max-to-segment"
#define TASKEX ""
...
struct SegTree {
  ...
  void PushDown(int rt) {
    if (sign[rt]) {
      sign[rt << 1] = max(sign[rt << 1], sign[rt]);
      minVal[rt<<1] = max(minVal[rt<<1], sign[rt << 1]); 

      sign[rt << 1 | 1] = max(sign[rt << 1 | 1], sign[rt]);
      minVal[rt<<1 | 1] = max(minVal[rt<<1 | 1], sign[rt << 1 | 1]);
      sign[rt] = 0;
    }
  }
