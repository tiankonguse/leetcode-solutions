...
desc: 右边界按 sm 进行分组，相同组按左边界排序，不同组按右边界排序，分别对每个分组为起始位置的查询进行处理
*/
...
void Solver() {  //
  ...
  for (const auto& [qi, ql, qr] : querys) {
    const int sm_l = ql / sm;  //
    const int sm_r = qr / sm;  //
    ...
    if (pre_sm_l > sm_l) {  // [sm_l+1, pre_sm_l] block 都可以进行持久化
      // printf("add block \n");
      for (int i = minLeft[pre_sm_l + 1] - 1; i / sm > sm_l; i--) {
        auto [u, v] = edges[i];
        dsu.Union(u, v);
      }
      pre_sm_l = sm_l;
