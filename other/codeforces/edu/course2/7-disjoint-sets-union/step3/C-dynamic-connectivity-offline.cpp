...
*/
#define TASK "C-dynamic-connectivity-offline"
#define TASKEX ""
...
void Work(const int l, const int r, const vector<tuple<char, int, int, int, int>>& queries) {
  if (l == r) {
    dsu.Persist();
    for (auto [op, L, R, u, v] : queries) {
      if (op == '?' && L == l) {
        ans[R] = dsu.Count();
      } else if (op == '.' && L <= l && r <= R) {
        dsu.Union(u, v);
  ...
  dsu.Persist();
  vector<tuple<char, int, int, int, int>> queries2;
  for (const auto& [op, L, R, u, v] : queries) {
    if (op == '.' && L <= l && r <= R) {
      dsu.Union(u, v);
    } else if ((op == '?' && l <= L && L <= r) || (op == '.' && Intersects(l, r, L, R))) {
      queries2.push_back({op, L, R, u, v});
    }
  }
