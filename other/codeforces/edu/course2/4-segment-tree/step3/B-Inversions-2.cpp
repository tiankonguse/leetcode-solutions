/*
ID: tiankonguse
TASK: B. Inversions 2  B. 倒转 2
LANG: C++
MAC EOF: ctrl+D
link:
https://codeforces.com/edu/course/2/lesson/4/3/practice/contest/274545/problem/B
PATH: ITMO 学院：试点课程 » 线段树，第 1 部分 » 步骤3 » 实践
submission: https://codeforces.com/edu/course/2/lesson/4/3/practice/contest/274545/submission/298572453
desc: n log(n) log(n)
*/
#define TASK "B-Inversions-2"
#define TASKEX ""
...
struct SegTree {
  ...
  void Build(int l = 1, int r = maxNM, int rt = 1) {
    if (l == r) {
      sumVal[rt] = r < maxNM ? 1 : 0;  // 最后一个是桩，需要设置为0
      return;
    }
    int m = (l + r) >> 1;
    Build(lson);
...
void Solver() {  //
  ...
  for (int i = n; i >= 1; i--) {
    int num = nums[i - 1] + 1;  // 查找 [l,r) 使得满足 num 个 1
    int l = 1, r = n + 1;
    while (l < r) {
      int mid = (l + r) / 2;
      if (segTree.QuerySum(mid, n) < num) {
        r = mid;
      } else {
        l = mid + 1;
      }
