...
submission: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/submission/298493736
desc: 二分查找，复杂度 log(n)*log(n) 280 ms	
*/
#define TASK "B-k-th-one-bianry"
#define TASKEX ""
...
int Solver(int k) {      //
  ...
  while (l < r) {
    int mid = (l + r) / 2;
    int num = segTree.QuerySum(1, mid);
    if (num < k) {
      l = mid + 1;
    } else {
      r = mid;
