/*
ID: tiankonguse
TASK: C. Number of Inversions on Segment C. 段上的反转次数
LANG: C++
MAC EOF: ctrl+D
link:
https://codeforces.com/edu/course/2/lesson/4/4/practice/contest/274684/problem/C
PATH: ITMO 学院：试点课程 » 线段树，第 1 部分 » 步骤4 » 实践
submission: https://codeforces.com/edu/course/2/lesson/4/4/practice/contest/274684/submission/298742592
*/
#define TASK "C-number-of-Inversions-on-segment"
#define TASKEX ""
...
void Merge(const Matrix& a, const Matrix& b, Matrix& c) {
  ...
  c.mat[0] = 0;
  for (int i = 1; i < N; i++) {
    preSum[i] = preSum[i - 1] + b.mat[i];
    c.mat[i] = a.mat[i] + b.mat[i];
  }
  for (int i = 1; i < N; i++) {
    ll num = a.mat[i];
    c.num += num * preSum[i - 1];
  }
}
typedef long long ll;
struct SegTree {
