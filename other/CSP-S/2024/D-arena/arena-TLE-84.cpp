...
submission: https://www.luogu.com.cn/record/220016400
最后三组数据超时
*/
#define TASK "arena"
#define TASKEX ""
...
pair<int, int> Dfs(const int p, const int R, const int minVal) {
  ...
  if (p >= dataOffset) {
    assert(R == 0);  // 此时 R 必须是 0
    assert(leftNo == A[p].second);
    if (p <= queryOffset) {
  ...
  if (queryOffset >= midOffset) {               // 左子树是确定的
    ...
    if (v == 1) {  // 由右树确定答案
      ...
      auto [rightMinWinVal, rightRangeWinVal] = Dfs(p * 2 + 1, R - 1, max(minVal, R));
      assert(rightMinWinVal == 0);
      assert(rightRangeWinVal == 0);
