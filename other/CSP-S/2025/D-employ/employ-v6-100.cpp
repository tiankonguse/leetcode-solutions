...
mint dp[2][N][N];  // 前 i 个人，j 个挂，其中 c 大于 j 的有 k 个
// 定义：只有 j 变化时，才计算排列数。
      ...
      for (int k = 0; k <= i; k++) {
        if (S[i + 1] == '1') {
          ...
          for (int t = 0; t <= min(c[j + 1], k); t++) {  // 枚举之前 k 个人中，有多少个忍耐度等于 j+1
            ...
            if (pre[j + 1] >= i - k + t) {
              dp[nextIndex][j + 1][k - t] += dp[index][j][k] * (pre[j + 1] - (i - k + t)) * C(c[j + 1], t) * A(k, t);
            }
            // 情况2：选择 > j+1 的人，此时 k 变为 k - t + 1
            dp[nextIndex][j + 1][k - t + 1] += dp[index][j][k] * C(c[j + 1], t) * A(k, t);
          }
        }
      }
    }
