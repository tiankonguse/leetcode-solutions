...
void Solver() {  //
  ...
  for (int i = 0; i < q; i++) {
    ...
    MyPrintf("i=%d offsetRange[%d, %d) \n", i, queries[i].offsetLeft, queries[i].offsetRight);
  ...
  for (int i = 0; i < n; i++) {
    ...
    // r1 一定小于 r2
    assert(r1 < r2);
    MyPrintf("i=%d offset1=%d offset2=%d r1=%d r2=%d\n", i, patterns[i].offset1, patterns[i].offset2, r1, r2);
  ...
  for (int rank = 0; rank < totalLen; rank++) {
    ...
    ans[queryIdx] += fenwick.Query(rank, rank);
  }
  for (int i = 0; i < q; i++) {
    printf("%lld\n", ans[i]);
  }
