...
vector<ll> townCosts;
vector<vector<pair<ll, int>>> townEdgeCosts;
Dsu dsu;
...
void Input() {
  ...
  townCosts.resize(K);
  townEdgeCosts.resize(K, vector<pair<ll, int>>(n));
  for (int k = 0; k < K; k++) {
    scanf("%lld", &townCosts[k]);
    for (int j = 0; j < n; j++) {
      ll w;
      scanf("%lld", &w);
      townEdgeCosts[k][j] = {w, j};
    }
...
tuple<ll, int, int> PopQue() {
  auto [w, k, index] = que.top();
  que.pop();
  if (index + 1 < n) {
    que.push({townEdgeCosts[k][index + 1].first, k, index + 1});
  }
  return {w, n + k, townEdgeCosts[k][index].second};
}

ll MinimumSpanningTree(ll cost) {
  dsu.Init(n + K);
  int selectIndex = 0, selectSize = selectEdges.size();
  auto MinEdge = [&]() -> tuple<ll, int, int> {
    if (selectIndex == selectSize) {
      return PopQue();
    }
    if (que.empty()) {
      return selectEdges[selectIndex++];
    }
    if (get<0>(selectEdges[selectIndex]) <= get<0>(que.top())) {
      return selectEdges[selectIndex++];
    }
    return PopQue();
  };
  int groupNum = n + K;
  while (selectIndex < selectSize || !que.empty()) {
    auto [w, u, v] = MinEdge();
    // if (w > maxEdge) {  // 不可能遇到
    //   assert(w > maxEdge);
    // }
  ...
  for (int sub = MASK; sub; sub = (sub - 1) & MASK) {
    ...
    for (int k = 0; k < K; k++) {
      if ((sub >> k) & 1) {
        ...
        que.push({townEdgeCosts[k][0].first, k, 0});
      }
    }
