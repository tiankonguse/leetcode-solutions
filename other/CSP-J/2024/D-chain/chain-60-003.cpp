...
https://www.luogu.com.cn/record/219574673
*/
#define TASK "chain"
...
vector<ll> preFlag;  // 上一轮接龙时，记录每个值结尾的有哪些人，-1 代表有多个人，否则是人的编号
vector<pair<ll, ll>> preList;
...
void Merge(ll R) {
  ...
  for (ll i = 0; i < n; i++) {
    ...
    for (ll j = piLeft; j < piRight; j++, offset++) {
      ...
      sum += flags[offset];
      if (sum > 0) {
        preList.push_back({S, i});  // 记录这个值可以结尾的人
        if (preFlag[S] == -1) {
...
void Solver() {  //
  ...
  while (t--) {
    ...
    R = 1;
    queries.resize(q);  // 预留空间，避免多次 realloc
    for (ll i = 0; i < q; i++) {
      ll r, v;
      scanf("%lld%lld", &r, &v);
    ...
    preList.clear();
    preList.push_back({1, -1});                // 初始状态，1 号值可以开始接龙
    ...
    for (ll r = 1; r <= R; r++) {              // 进行 R 轮游戏
      ...
      for (auto [v, pos] : preList) {
        preFlag[v] = -1;  // 清空上一轮接龙的标记
