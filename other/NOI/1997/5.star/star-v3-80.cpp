...
void Solver() {  //
  ...
  for (auto [x, y, z, r] : nums) {
    const auto xit = X.lower_bound(x);
    const auto yit = Y.lower_bound(y);
    const auto zit = Z.lower_bound(z);
    // 左下角为 (x,y,z) 边长为 r 的正方体被三个平面列表 X, Y, Z 切割为若干个长方体 (x,y,z, xr, yr, zr)
    for (auto it = xit; *it < x + r; it++) {
      for (auto it2 = yit; *it2 < y + r; it2++) {
        for (auto it3 = zit; *it3 < z + r; it3++) {
          int x0 = *it;
          int y0 = *it2;
          int z0 = *it3;
          ll h = (ll)x0 * 10000 * 10000 + y0 * 10000 + z0;
          if (H.count(h) == 0) {
            int xr = *next(it) - x0;
            int yr = *next(it2) - y0;
            int zr = *next(it3) - z0;
