...
map<tuple<int, int, int, int, int, int>, int> H;
set<int> X, Y, Z;
void Solver() {  //
  scanf("%d", &n);
  for (int i = 0; i < n; i++) {
  ...
  for (auto [x, y, z, r] : nums) {
    ...
    for (auto it = X.lower_bound(x); *it < x + r; it++) {
      for (auto it2 = Y.lower_bound(y); *it2 < y + r; it2++) {
        for (auto it3 = Z.lower_bound(z); *it3 < z + r; it3++) {
          int x0 = *it;
          int y0 = *it2;
          int z0 = *it3;
          int xr = *next(it) - x0;
          int yr = *next(it2) - y0;
          int zr = *next(it3) - z0;
          H[make_tuple(x0, y0, z0, xr, yr, zr)]++;
        }
      }
    }
  }
  int ans = 0;
  for (const auto& [k, cnt] : H) {
    auto [x, y, z, xr, yr, zr] = k;
    ans += xr * yr * zr;
  }
  printf("%d\n", ans);
}
