...
ll dp0[111][111][3];  // dp0[r][m][s] 代表前r个元素，组成m段, 最后一个元素选择 S秒的最大高度和

ll Dfs0(const int r, const int k, const int s) {
  int rn = r + 1;
  if (k == 0) return 0;
  if (rn < k) return -2;  // 元素不够分段
  ll& ret = dp0[r][k][s];
  if (ret != -1) return ret;
  ...
  for (int last = 0; last < r; last++) {
    for (int p = 0; p < 3; p++) {
      const int pa = nums[last][p];
      const int pb = nums[last][(p + 1) % 3];
      const int pc = nums[last][(p + 2) % 3];
      // r 自己一段
      ll tmp = Dfs0(last, k - 1, p);
      if (tmp != -2) {
        ret = max(ret, tmp + sc);
      }
