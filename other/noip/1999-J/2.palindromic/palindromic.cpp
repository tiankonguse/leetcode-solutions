...
unordered_map<char, int> mpToVal;
unordered_map<int, char> mpToChar;
void Init() {
  for (int i = 0; i < 10; i++) {
    mpToVal['0' + i] = i;
    mpToChar[i] = '0' + i;
...
int ToVal(char c) { return mpToVal[c]; }
char ToChar(int v) { return mpToChar[v]; }
...
void Add(string& s) {
  ...
  int carry = 0;
  for (int i = 0; i < n; i++) {
    int sum = ToVal(s[i]) + ToVal(rs[i]) + carry;
    s[i] = ToChar(sum % N);
    carry = sum / N;
  }
  if (carry) {
