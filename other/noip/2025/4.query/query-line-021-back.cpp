...
void SolverSquare(int L, int a) {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    Q1.pop(i);
    if (i + a <= n) {
      Q1.push_back(preSums[i + a], i + a);
    }

    // Q2 min[i-L-a-1, i-L-1]
    Q2.pop(i - L - a - 1);
    if (i - L - 1 >= 0) {
      Q2.push_back(preSums[i - L - 1], i - L - 1);
    }
    ll maxVal = Q1.front();
    ll minVal = Q2.front();
...
void Init() {
  ...
  for (int i = 0; i <= n; i++) {
    rmqMax[0][i] = preSums[i];
    rmqMin[0][i] = preSums[i];
  }
  int maxLog = log2(n) + 1;
  for (int k = 1; k < maxLog; k++) {
    for (int i = 0; i <= n; i++) {
      if (i + (1 << (k - 1)) > n) break;
      rmqMax[k][i] = max(rmqMax[k - 1][i], rmqMax[k - 1][i + (1 << (k - 1))]);
      rmqMin[k][i] = min(rmqMin[k - 1][i], rmqMin[k - 1][i + (1 << (k - 1))]);
    }
  }
  base2[1] = 0;
  for (int i = 2; i <= n; i++) {
