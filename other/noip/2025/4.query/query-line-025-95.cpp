...
inline int Log(int x) { return __lg(x); }
const int kMaxLog = 16;

namespace SparseTableMax {
ll st[kMaxLog][max4];
...
inline ll MaxSum(const int a, const int b) { return SparseTableMax::Query(a, b); }
inline ll MinSum(const int a, const int b) { return SparseTableMin::Query(a, b); }
...
void Init() {
  // InitBase2(n);
  SparseTableMax::Init(n, preSums);
  SparseTableMin::Init(n, preSums);
}
