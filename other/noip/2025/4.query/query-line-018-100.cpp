...
ll rmqMax[kMaxLog][max4];
ll rmqMin[kMaxLog][max4];
...
inline ll MaxSum(const int a, const int b) {
  ...
  return max(rmqMax[k][a], rmqMax[k][b - (1 << k) + 1]);
}
inline ll MinSum(const int a, const int b) {
  const int k = base2[b - a + 1];
  return min(rmqMin[k][a], rmqMin[k][b - (1 << k) + 1]);
}

inline void SolverLEx(const int q, const int i, const int D, const int U, const int W) {
  auto& Qq = Q[q];
  ...
  Qq.pop(i - W);
  if (i + D <= n) {
    ll tmp = MaxSum(i + D, min(i + U, n)) - preSums[i - 1];
    Qq.push_back(tmp, i);  // add dpL[i-minDis]
  }
}
inline void SolverR(const int q, const int i, const int D, const int U, const int W) {
  auto& Qq = Q[q];
...
ull Solver(int L, int R) {
  ...
  for (int i = 1; i <= n; i++) {
    SolverLEx(0, i, L - 1, R - 1, L - 1);             // 计算梯形的左半部平行四边形
    SolverLEx(1, i, L - 1 + a, R - 1, a + L - 1);     // 计算三角形左半部平行四边形
    SolverR(2, i, L - 1 + a, R - 1, a);               // 计算三角形右半部平行四边形
    ll v1 = Q[0].empty() ? INT64_MIN : Q[0].front();  // 梯形的左半部平行四边形
    ll v2 = Q[1].empty() ? INT64_MIN : Q[1].front();  // 三角形左半部平行四边形
    ll v3 = Q[2].empty() ? INT64_MIN : Q[2].front();  // 三角形右半部平行四边形
...
void Init() {
  ...
  for (int k = 1; k < maxLog; k++) {
    ...
    for (int i = 0; i + (1 << k) - 1 <= n; i++) {
      rmqMax[k][i] = max(rmqMax[k - 1][i], rmqMax[k - 1][i + len]);
      rmqMin[k][i] = min(rmqMin[k - 1][i], rmqMin[k - 1][i + len]);
    }
  }
  base2[1] = 0;
  for (int i = 2; i <= n; i++) {
...
void Solver() {  //
  scanf("%d", &n);
  rmqMin[0][0] = rmqMax[0][0] = preSums[0] = 0;
  for (int i = 1; i <= n; i++) {
    scanf("%lld", &preSums[i]);
    preSums[i] += preSums[i - 1];
    rmqMax[0][i] = rmqMin[0][i] = preSums[i];
  }
  Init();
  scanf("%d", &q);
  while (q--) {
