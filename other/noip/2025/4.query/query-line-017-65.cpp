...
int n, q;
vector<ll> preSums, base2;
vector<vector<ll>> rmqMax, rmqMin;
...
inline ll SolverLEx(const int q, const int j, const int D, const int U, const int W) {
  auto& Qq = Q[q];
...
inline ll SolverR(const int q, const int i, const int D, const int U, const int W) {
  auto& Qq = Q[q];
...
void Init() {
  int maxLog = log2(n) + 1;
  rmqMax.resize(n + 1, vector<ll>(maxLog, INT64_MIN));
  rmqMin.resize(n + 1, vector<ll>(maxLog, INT64_MAX));
  base2.resize(n + 1);
