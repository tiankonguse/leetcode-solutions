...
tuple<int, int, int, int, bool> TryFalse(int nameId, int week, int notWeek, int guilty, int notGuilty) {
  for (const auto& state : nameToWords[nameId]) {
    ...
    if (state.type == GUILTY) {
      ...
      guilty |= 1 << xxx;
      if ((notGuilty & guilty) || (guilty == (1 << m) - 1)) {
        return {week, notWeek, guilty, notGuilty, false};
      }
    } else if (state.type == DAY) {
      notWeek |= 1 << xxx;
      // 不能 7 天都是 notWeek
      if ((notWeek & week) || (notWeek == 0x7F)) {
        return {week, notWeek, guilty, notGuilty, false};
      }
    }
  }
...
void Dfs(int p, int num, int week, int notWeek, int guilty, int notGuilty) {
  if (p < num) return;
  if (p == 0) {
    UpdateAns(week, notWeek, guilty, notGuilty);
    return;
  }
  // 说真话
  auto [trueWeek, trueNotWeek, trueGuilty, trueNotGuilty, trueOK] = TryTrue(p, week, notWeek, guilty, notGuilty);
  if (trueOK) {
    Dfs(p - 1, num, trueWeek, trueNotWeek, trueGuilty, trueNotGuilty);
  }

  if (num > 0) {
    // 说假话
    auto [falseWeek, falseNotWeek, falseGuilty, falseNotGuilty, falseOK] =
        TryFalse(p, week, notWeek, guilty, notGuilty);
    if (falseOK) {
      Dfs(p - 1, num - 1, falseWeek, falseNotWeek, falseGuilty, falseNotGuilty);
    }
  }
}
