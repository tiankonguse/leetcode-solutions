...
const unordered_map<string, int> weeks = {{"Monday", 0}, {"Tuesday", 1},  {"Wednesday", 2}, {"Thursday", 3},
...
void Parse(string s) {
  ...
  const string words = s.substr(pos + 1);
...
void UpdateAns(int today, int guilty, int notGuilty) {}

tuple<int, int, int, bool> TryTrue(int nameId, int today, int guilty, int notGuilty) {
  for (const auto& state : nameToWords[nameId]) {
    if (state.type == GUILTY) {
      int xxx = state.xxx;
      guilty |= 1 << xxx;
      if (notGuilty & (1 << xxx)) {
        return {today, guilty, notGuilty, false};
      }
    } else if (state.type == NOT_GUILTY) {
    } else if (state.type == DAY) {
      if (today == -1 || today == state.xxx) {
        today = state.xxx;
      } else {
        return {today, guilty, notGuilty, false};
      }
  ...
  return {today, guilty, notGuilty, true};
}

tuple<int, int, int, bool> TryFalse(int nameId, int today, int guilty, int notGuilty) {
  for (const auto& state : nameToWords[nameId]) {
    // TODO: implement logic for TryFalse
  }
  return {today, guilty, notGuilty, true};
}

// 深度优先搜索，前 p 个人有 num 个人说谎
void Dfs(int p, int num, int today, int guilty, int notGuilty) {
  if (p < num) return;
  if (p == -1) {
    UpdateAns(today, guilty, notGuilty);
    return;
  }
  // 说真话
  auto [trueToday, trueGuilty, trueNotGuilty, trueOK] = TryTrue(p, today, guilty, notGuilty);
  if (trueOK) {
    Dfs(p - 1, num, trueToday, trueGuilty, trueNotGuilty);
  }

  if (num > 0) {
    // 说假话
    auto [falseToday, falseGuilty, falseNotGuilty, falseOK] = TryFalse(p, today, guilty, notGuilty);
    if (falseOK) {
      Dfs(p - 1, num - 1, falseToday, falseGuilty, falseNotGuilty);
    }
  }
}
