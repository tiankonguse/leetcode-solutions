        ...
        for(int i=0, j=0;i<n;i++){
           ...
           // printf("i=%d now=%d\n", i, now);
            ...
            while(j < n && tasks[j].inTime <= now){
                que.push(tasks[j]);
                //printf("que add: j=%d index=%d t=%d inTime=%d\n", j, tasks[j].index, tasks[j].t, tasks[j].inTime);
                j++;
            }
            ...
            now += task.t;
            //printf("pop index=%d t=%d inTime=%d now=%d\n", task.index, task.t, task.inTime, now);
        }
        return ans;
    }
};
