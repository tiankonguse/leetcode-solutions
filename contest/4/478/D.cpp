...
void Solver(vector<int>& A, vector<MoQueue::Query>& queries) {
  ...
  MyPrintf("total_ops: %lld \n", total_ops);
  for (auto& q : queries) {
    // L 移动
    while (current_l > q.l) {
      current_l--;
      MyPrintf("add current_l: %d, B=%d\n", current_l, B[current_l]);
      Add(B[current_l]);
    }
    while (current_l < q.l) {
      Remove(B[current_l]);
      MyPrintf("remove current_l: %d, B=%d\n", current_l, B[current_l]);
      current_l++;
    }
    ...
    while (current_r < q.r) {
      current_r++;
      MyPrintf("add current_r: %d B=%d\n", current_r, B[current_r]);
      Add(B[current_r]);
    }
    while (current_r > q.r) {
      Remove(B[current_r]);
      MyPrintf("remove current_r: %d B=%d\n", current_r, B[current_r]);
      current_r--;
    }

    // 存储答案
    q.answer = total_ops;
    MyPrintf("q: [%d %d], idx=%d, answer=%lld\n", q.l, q.r, q.id, q.answer);
  }
}
