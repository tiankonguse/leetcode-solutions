...
struct SegTree {
  ...
  //   int m = (l + r) >> 1;
  //   if (L <= m) {
  //     UpdateForce(L, R, setVal, lson);
  //   } else if (R > m) {
  //     UpdateForce(L, R, setVal, rson);
  //   }
  //   PushUp(rt, l, r);
  // }
    ...
    if (L <= l && r <= R) {
      if (setVal > rightUpVal[rt]) {    // 更新最大值
        if (setVal >= leftUpVal[rt]) {  // 更新所有的最大值
          sign[rt].first = UP;
          sign[rt].second = leftUpVal[rt] = rightUpVal[rt] = setVal;
        ...
        if (setVal < leftDownVal[rt]) {        // 更新所有的最小值
          sign[rt].first = Down;
          sign[rt].second = leftDownVal[rt] = rightDownVal[rt] = setVal;
