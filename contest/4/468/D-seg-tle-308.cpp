...
struct SegTree {
  ...
  inline int Num(const pair<ll, ll>& p) { return p.second - p.first + 1; }
  // 合并函数，按需进行合并
  ...
  void UpdateOne(int L, int R, ll setVal, int l = 1, int r = maxNM, int rt = 1) {
    if (L <= l && r <= R) {
    ...
    sign[rt] = -1;  // 初始化，标记无效，直接清理
    int m = (l + r) >> 1;
    if (L <= m) UpdateOne(L, R, setVal, lson);
    if (R > m) UpdateOne(L, R, setVal, rson);
    PushUp(rt, l, r);
  }
  ...
  void UpdateMax(int L, int R, ll setVal, int l = 1, int r = maxNM, int rt = 1) {
    ...
    PushDown(rt);  // 有标记，代表区间的值永远相等，不需要递归更新
    int m = (l + r) >> 1;
...
class Solution {
  ll n;
  vector<int> nums;
  pair<ll, ll> SearchAndCheck(ll d, ll K) {  // 大于等于 d 的子数组的个数与值的和

    ll num = 0;
    ll sum = 0;
    for (int i = 1; i <= n; i++) {
      const ll nowVal = nums[i - 1];
      segTreeMax.UpdateOne(i, i, nowVal);  // 强制刷新，代替初始化
      segTreeMin.UpdateOne(i, i, nowVal);
 ...
 public:
  ll maxTotalValue(vector<int>& nums_, const int k) {  //
    ...
    auto [numR, sumR] = SearchAndCheck(r, INT64_MAX);
