...
class Solution {
 ...
 public:
  double minTime(int n_, int k_, int m_, vector<int>& time_, vector<double>& mul_) {
    ...
    min_queue<tuple<double, int, int>> que;
    que.push({0, MASK - 1, 0});
    dp1[MASK - 1][0] = 0;
    double ans = 1e18;
    while (!que.empty()) {
      const auto [t, mask, mi] = que.top();
      que.pop();
      ...
      for (int sub = mask; sub; sub = (sub - 1) & mask) {
        ...
        if (sub == mask) {  // 全过去了，得到一个答案
          ...
          for (int i = 0; i < n; i++) {
            ...
            if ((maskBack & (1 << i)) == 0) {
              double returnTime = time[i] * mul[goMi];
              const double returnT = T + returnTime;
              const int returnMi = nextLoop(goMi, returnTime);
              const int maskReturn = maskBack | (1 << i);
              if (returnT < dp1[maskReturn][returnMi]) {
                dp1[maskReturn][returnMi] = returnT;
                que.push({returnT, maskReturn, returnMi});
              }
            }
          }
        }
