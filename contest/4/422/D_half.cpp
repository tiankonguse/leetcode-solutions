...
class Solution {
  ...
  ll Dfs(const ll V, const ll S0, const ll N0) {
    if (S0 > sum2 || N0 > EventAll) return 0;
    if (V == -1) {
      if (S0 == sum2 && N0 == EventAll) {
        return 1;
      }
      return 0;
    }
    ...
    const ll K = nums[V];
    if (K == 0) {
      return ret = Dfs(V - 1, S0, N0);
    }

    const ll P = n - counts[V];  // [V+1,9] 已选择的数字个数
    const ll selectEven = N0;
    const ll selectOdd = P - N0;
    ...
    for (ll i = 0; i <= K; i++) {
      ...
      const ll nextAns = Dfs(V - 1, S0 + trySelectEven * V, N0 + trySelectEven);
