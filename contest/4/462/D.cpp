...
class Solution {
  ...
  bool DfsAll(const int len, const int mid, const int offset, const ll val) {
    if (offset == len / 2) {
      MyPrintf("len=%d, mid=%d, offset=%d, val=%lld\n", len, mid, offset, val);
      // 先检查 数字中每个数字 k 出现 恰好 k 次
      if (mid > 0 && bits[mid] != mid) {
        MyPrintf("false: mid > 0 && bits[mid]!= mid\n");
        return false;
      }
      for (int i = 2; i < 10; i += 2) {
        if (bits[i] > 0 && bits[i] != i) {
          MyPrintf("false: i=%d bits[i]=%d bits[i]!= i \n", i, bits[i]);
          return false;
        }
      }
      if (this->n < val) {
        MyPrintf("true: this->n[%lld] < val[%lld]\n", this->n, val);
        dfsAns = val;
        return true;
      } else {
        MyPrintf("false: this->n[%lld] >= val[%lld]\n", this->n, val);
        return false;
      }
    }
