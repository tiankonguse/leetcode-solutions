...
class Solution {
 ...
 public:
  ll maxSum(vector<int>& nums, const int K) {  //
    ...
    map<pair<ll, ll>, int> rank;
    for (int i = 1; i <= n; i++) {
      rank[sortedNums[i - 1]] = i;
    }
    ll ans = nums.front();
    // len = 1, 等于最大值
    ...
    for (int len = 2; len < n; len++) {
      ...
      for (int i = 1; i <= n; i++) {
        ...
        const int idxRight = rank[rightNode];

        segTreeWindows.Set(idxRight, rightVal);
        window.insert(rightNode);
        sumWindow += rightVal;

        segTreeOther.Unset(idxRight);
        other.erase(rightNode);
        sumOther -= rightVal;
        ...
        if (left >= 1) {
          ...
          const int idxLeft = rank[leftNode];

          segTreeWindows.Unset(idxLeft);
          window.erase(leftNode);
          sumWindow -= leftVal;

          segTreeOther.Set(idxLeft, leftVal);
          other.insert(leftNode);
          sumOther += leftVal;
        }
        MyPrintf("-------\n");
        MyPrintf("len = %d, (%d, %d], sumWindow = %lld, sumOther = %lld\n", len, left, right, sumWindow, sumOther);
        MyPrintf("window: ");
        for (auto [val, idx] : window) {
          MyPrintf("(%lld, %d) ", val, idx);
        }
        MyPrintf("\n");
        MyPrintf("other: ");
        for (auto [val, idx] : other) {
          MyPrintf("(%lld, %d) ", val, idx);
        }
        MyPrintf("\n");
        // 窗口最小值 不小于 其他最大值，不需要交换
        if (window.begin()->first >= other.rbegin()->first) {
          MyPrintf("不需要交换, ans = %lld\n", sumWindow);
          ans = max(ans, sumWindow);
          continue;
        }

        const int windowsSize = window.size();
