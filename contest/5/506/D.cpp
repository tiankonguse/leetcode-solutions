...
struct SegTree {
  ...
  void Init(int n) {
    maxNM = n + 1;
    vals.resize(kMaxN << 2);
    sumVal.resize(kMaxN << 2);
    countVal.resize(kMaxN << 2);
  }
  // 合并函数，按需进行合并
...
vector<pair<ll, ll>> sortedNums(2500);
vector<int> gRank(2500);
class Solution {
  // 返回最大子数组和
  ...
  ll maxSum(vector<int>& nums, const int K) {  //
    ...
    for (int len = 2; len < n; len++) {
      ...
      for (int i = 1; i <= n; i++) {
        ...
        const int idxRight = gRank[right];
