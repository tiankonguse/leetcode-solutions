 ...
 public:
  int minCostToEqualizeArray(vector<int>& nums, ll cost1, ll cost2) {
    ...
    while (que.top() < maxVal) {  // 复杂度 O(n)
      ...
      if (b < maxVal) {
        ll dis = (maxVal - b + 2) / 3;  // 加速优化，每次操作，有一个等于maxVal
        ans2 += dis;
        que.push(a + dis);
        que.push(b + dis);
      } else {
    ...
    if (que.top() == maxVal) {  // 只使用 cost2 找到答案
      return Solver(ans1, ans2);
    }
    if (n == 2) {
      return Solver(ans1 + maxVal - que.top(), ans2);
    }
    // 其他的都是 maxVal， 最后一个小于 maxVal

    auto [newAns1, newAns2] = Solver2(n, maxVal, que.top());
